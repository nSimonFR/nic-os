...
Create a read-only, evidence-backed guide for piloting a finished Commander deck. It explains game plan, sequencing, mulligans, interaction, win conditions, politics, and key rules interactions. It does not make cuts or additions; route deck changes to `mtg-commander-deckbuilding`.
...
Do not use for deck construction, upgrades, validation, or collection filtering; load `mtg-commander-deckbuilding` instead.

## Workflow

1. **Acquire and verify the list.** Fetch the supplied public deck with `mcp__mtg__get_moxfield_deck` or `mcp__mtg__get_archidekt_deck`. Confirm name, format, commander(s), command zone, and mainboard before analysing. If the commander is not clear, ask; never infer it from the first listed card.
