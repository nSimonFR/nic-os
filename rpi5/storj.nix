...
in
{
  ...
  systemd.services.rclone-storj-sync = {
    ...
    serviceConfig = {
      Type      = "oneshot";
      ExecStart = "${pkgs.rclone}/bin/rclone sync ${mountPoint} storj:rpi5-cloud --config /run/agenix/rclone-storj --transfers 4 --create-empty-src-dirs --max-size 25G --exclude lost+found/**";
    };
  };
