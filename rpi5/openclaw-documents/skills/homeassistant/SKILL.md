...
metadata: {"openclaw":{"emoji":"🏠","requires":{"bins":["curl"],"env":["HA_TOKEN"]},"primaryEnv":"HA_TOKEN"}}
---
...
- `HA_TOKEN`: Long-lived access token (create in HA → Profile → Long-Lived Access Tokens, or read from `~/.secrets/ha-token` if stored there)
...
- HA runs locally on this RPi5 — no external network needed
- API returns JSON by default
- Long-lived tokens don't expire — store securely in `~/.secrets/`
