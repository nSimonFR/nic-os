...
| [infisical-cli](/Users/nsimon/MyDocuments/TRUSK/notes/infisical-cli.md) | `inf-stg` / `inf-prod` · `infisical: command not found` · CLI prints an empty table, no error · `302` to `staging-auth` / `prod-auth` · `inf: … expired` · read or set a staging/prod secret · `infisical run` locally · which `--projectId` / `--env` · refresh `INFISICAL_*_TRUSK` |
