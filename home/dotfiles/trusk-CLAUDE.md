...
| [merge-and-ci-traps](/Users/nsimon/MyDocuments/TRUSK/notes/merge-and-ci-traps.md) | `gh pr merge` refused · "can't be rebased" · CI red but local green · `mockResolvedValue(null)` · peer-dep resolution · `-pr.` version pins · e2e `Failed to connect before the deadline` · pod pulls `nginx:1.0.0` · chart values ignored / alias unresolved · `deployment/charts/charts/*.tgz` · zsh loop over `$apps` / `$files` does nothing · `recent account payments have failed` · job failed with 0 steps · merge cut no release / no image · PR shows `no checks reported` · `CONFLICTING` after a release · `git stash pop` |
| [schema-migrations](/Users/nsimon/MyDocuments/TRUSK/notes/schema-migrations.md) | editing anything under `schema/migrations` · `42703 column does not exist` after deploy · `_migrations` table · pg-migrate → TypeORM · `_knex_migrations` · `migration:run --fake` · init container replays `CREATE TABLE` · `migration:run` crashes on an entity (`OmitType` / `prototype`) |
| [field-rename-releases](/Users/nsimon/MyDocuments/TRUSK/notes/field-rename-releases.md) | renommer/scinder un champ lu par d'autres services · retirer une double écriture · étape *contract* d'un expand/contract · « on a bien tous les consommateurs ? » · `filter.<ancien_nom>` · zéro résultat sans erreur après un déploiement · `throwOnInvalidFilter` · `nestjs-paginate` ignore un filtre en silence |
...
| [prod-vs-staging-prerequisites](/Users/nsimon/MyDocuments/TRUSK/notes/prod-vs-staging-prerequisites.md) | bump prod isolé d'un service Nest 11 · `FailedMount` / `CreateContainerConfigError` sur `/etc/trusk-auth` · rollout prod bloqué en `ContainerCreating` · sidecar flagd qui ne démarre pas · `flagd/shared-flags` introuvable · « marche en staging, pas en prod » · `TRUSK_AUTH_ISSUER` absent · trusk-app < 0.15.0 · `service tokens disabled` · un service ne signe pas ses appels sortants |
| [staging-access-and-writes](/Users/nsimon/MyDocuments/TRUSK/notes/staging-access-and-writes.md) | `i/o timeout` sur `tailscale-operator-staging` · `proxy-staging` · les deux tunnels sur `8888` · `x509: certificate signed by unknown authority` · « suis-je bien sur staging ? » avant une écriture · `permission denied for table` · correctif multi-schémas · `postgres_staging_rw` HS |
| [infisical-cli](/Users/nsimon/MyDocuments/TRUSK/notes/infisical-cli.md) | read, add, change or inject a service's secrets / env vars (Infisical is where Trusk secrets live, staging and prod) · `inf-stg` / `inf-prod` · `infisical: command not found` · CLI prints an empty table, no error · `302` to `staging-auth` / `prod-auth` · `inf: … expired` · read or set a staging/prod secret · `infisical run` locally · which `--projectId` / `--env` · refresh `INFISICAL_*_TRUSK` |
| [preview-environments](/Users/nsimon/MyDocuments/TRUSK/notes/preview-environments.md) | `appset-preview` vs `pr-<slug>` · `pr-<n>-bo.trusk.com` · about to enable an authz gate / rate limit in staging · preview behaves unlike staging · « can we validate on a preview? » · no refusals but was there any traffic · a preview reads staging's flags · `flagd-preview` unread · eject a flag onto one preview · `auth.enforcement.bypassed` · a 403 in the logs but 200 on the wire · a service looks missing from a preview · `infra-env-infisical-appset-shared` · `env.values[0..3]` clobbered · preview à 0/64 pods · `SleepInfo` / kube-green · `wake-up-namespace` · `Sync operation blocked by sync window` · `pr-namespace-shutdown` · `data-bo` · amorcer des données de preview · `CreateContainerConfigError` après un réveil · `secret "staging-env" not found` · wake-up at night · ops stuck `Running` `blocked by sync window` · `initiatedBy.automated` · `pr-<slug>-gitops` · operation stuck, new sync ignored · OFREP `:8016` · which flag value does the pod really see · `FF_DISABLED` · `openfeature.dev/featureflagsource` · 1/1 without the flagd sidecar after a wake |
| [smoke-tests-qa](/Users/nsimon/MyDocuments/TRUSK/notes/smoke-tests-qa.md) | lancer un smoke test · `st-bo` / `st-e2e` / `st-business` · `trusk-automation` · tous les tests au rouge en ~8 s en local · `validate (node_modules/devtools…)` · `le scénario @createAPI doit passer avant` · `smoketest-e2e-template` · namespace `awf-qa` · `argo: command not found` · `/smoketest <staging>` · où atterrit le rapport Slack · valider un déploiement staging avant la prod |
...
| [datadog-prod-observability](/Users/nsimon/MyDocuments/TRUSK/notes/datadog-prod-observability.md) | « pas d'erreurs sur Datadog » comme preuve de santé · vérifier un déploiement prod dans les logs · pic d'erreurs juste après un rollout · `auth.enforcement.bypassed` en volume · monitor `[LOGS] — … burst` · comparer un taux d'erreurs à une baseline · `service:backoffice` vs `trusk-backoffice` · `@version` absent d'un service |
| [authz-rollout](/Users/nsimon/MyDocuments/TRUSK/notes/authz-rollout.md) | `*_backend_authz` · `@AuthRules` · TEC-301 · raising an authz flag · `Missing required permissions` · `Endpoint restricted to services` · 401 on a valid token · `services:` list / who is the caller `sub` · `communications-engine` · gateway strips `authorization` · `DEFAULT_ACCESS` · probe a service's rules on a preview · `.artifact-rights` · relayed token / `TEC-301 relay, to settle` · `runOutsideRequestContext` · `401 no-token` from a legacy caller · gateway `iamUserId` · data-bo + smoke with gates up · `/u/login/identifier` |
...
Datadog gotchas: **`env:production` and `namespace:production` are NOT searchable facets — but `kube_namespace:production` IS** (corrigé le 2026-09-07 ; l'ancienne formulation envoyait vers `host:gke-trusk-production*`, qui marche aussi mais oblige à énumérer les services et fait donc manquer ceux qu'on n'a pas anticipés). Version in scope is `@version` (log attribute) or `image_tag:` (pod tag). Prefer `aggregate` + `groupBy` for anything you'll quote as a number; `sample:"spread"` samples and must never be counted, and the **current time bucket is incomplete** so never conclude "it dropped to zero" on it. `search` with `sample:"diverse"` is the right tool to enumerate distinct error patterns.

Avant de conclure quoi que ce soit d'une requête de logs prod — surtout « pas d'erreurs » après un déploiement — lire [`notes/datadog-prod-observability.md`](/Users/nsimon/MyDocuments/TRUSK/notes/datadog-prod-observability.md) : quels services loggent vraiment, les trois familles de bruit à exclure, et pourquoi une baseline d'heure creuse ne vaut rien.
...
**Logs retention = 15 days** (single `main` index, no flex logs) — queries beyond that silently return nothing, so an "it started on <today − 15d>" onset date is the retention cliff, not a launch (bit us on the trusk-api 429 investigation, 2026-09-03). For any history question >15d, use `tech-datadog_metrics` (15-month retention), e.g. `sum:nginx_ingress.controller.requests{ingress:<svc>,status:429}.as_count()` — per-ingress + per-status, no user-agent dimension.
...
3. **Image** → `europe-west1-docker.pkg.dev/trusk-tools-tpfqef/trusk-registry/<service>:<version>`. Built by `ci.yaml` on the **master push of the `Chore(Version): <ver>` commit** — one digest tagged both `master` and `<version>`. Bumping trusk-applications before it exists ⇒ `ImagePullBackOff` + `not found` — expected, wait. Find it: `gh run list --repo trusk-official/<svc> --workflow ci.yaml` → the run whose `headSha` is the `Chore(Version)` commit.
   - **The tag-push run is a decoy: it comes out `skipped` on every repo since ~2026-08.** Verified 2026-09-08: trusk-api 3.33.2, state-status 1.39.0→1.41.0, order-mission 1.65.1, backoffice 1.395.1 — all `skipped`, all images present. (trusk-api 3.33.0/3.33.1, 28-30/07, still had `success`, hence the old note.) Don't wait on it, and don't `gh run rerun` it: jobs that were *skipped* rather than failed re-skip instantly (attempt 2 completes in seconds, still `skipped`).
   - **Check the image with `--filter="tag~<version>"`, never `tag=`.** gcloud prints `WARNING: tag=3.33.2 currently does not match but will match in the near future` and returns **nothing** — an exact-match probe reports the image absent while the pod is already running it. Cost me a false "image missing" on 2026-09-08.
   - Measured trusk-api 3.33.2, 2026-09-08 (UTC): merge 10:32:27 → Release Workflow 10:32:30→10:34:20 (cuts the tag, no build) → version-commit CI 10:34:04→**11:02:27** → image digest created 10:48:59 → staging pod ready 11:04:56. **≈32 min merge → pod live.**
   - The CI run on the *merge* commit is a separate run and can fail without blocking the release (flaky e2e on 2026-09-08: `PUT /organisation › Should throw 422 for invalid payload`, green on the version-commit run 20 min later). A red master CI does not mean the release is broken — check which of the two runs it is.
   - **A successful release does NOT mean the image exists.** Two pushes close together (a merge, then semantic-release's `Chore(Version)`) put two `ci.yaml` runs in the same concurrency group: GitHub **cancels** the version-commit one. The release, the tag and the CHANGELOG all look perfect, and the only image produced is tagged **`master` alone** — never `<version>`. Seen on state-status 1.42.1, 2026-09-15: merge and version commit 30 s apart, version-commit runs `cancelled` + `skipped`, registry showing `master` at 13:32 and `1.42.0` at 12:44, **no `1.42.1`**. Bumping trusk-applications on the release number alone ⇒ `ImagePullBackOff` on an image that will never come. **Assert the tag in the registry before every bump**, don't infer it from the release.
   - A *cancelled* run **does** re-run correctly (`gh run rerun <id>` → attempt 2 builds and tags properly) — unlike a *skipped* one, which re-skips instantly (previous bullet). So: check the conclusion before deciding whether a rerun is worth anything.
...
Writing: use the `postgres_staging_rw` MCP (`mcp_readwrite` holds `pg_write_all_data` → multi-schema fixes stay in one transaction). Only when it and the VPN are down do you fall back to per-service pods, where each app role writes its own schema only — non-atomic, see [`notes/staging-access-and-writes.md`](/Users/nsimon/MyDocuments/TRUSK/notes/staging-access-and-writes.md).
...
## Shell — le tool Bash tourne en **zsh**, qui ne découpe pas les variables

Pas de word splitting par défaut en zsh : `set -- $pair` laisse `"a b c"` **entier** dans `$1`, et `$2`/`$3` sont vides. Une boucle `for pair in "state-status 1.39.0 1.41.0" …; do set -- $pair; git -C "$1" grep … "$2"; done` ne signale rien et renvoie des résultats **silencieusement faux** — ça a produit des comptages tous à zéro sur les lectures de `derive_author` pendant la préparation de la MEP du 2026-09-07, ce qui aurait inversé la conclusion sur l'ordre des vagues. Utiliser :

```bash
printf '%s\n' "svc 1.2.3" "svc2 4.5.6" | while read -r svc from to; do … done
# ou:  read -r svc from to <<< "$pair"
```

Corollaire : toute boucle shell dont le résultat sert de preuve doit être vérifiée sur **un** cas à la main avant d'être crue.
...
Since ~2026-08 the direct push prints `remote: - Changes must be made through a pull request` **yet the commit can still land**. Confirm with `git fetch && git log origin/master -1` before retrying — retrying blind duplicates the bump. If it really bounced: branch + PR + `--admin`.


## kubectl contexts

- **Staging** — `trusk-staging-ts` (Tailscale operator). **Two tailscaled run side by side on this Mac**: the macOS app holds the *personal* tailnet, and a nix-managed daemon (`launchd.daemons."tailscale-work"`, `hosts/nbookpro/configuration.nix`) holds the *Trusk* one, in userspace mode. A bare `tailscale status` queries the personal one, finds no `tailscale-operator-staging`, and every kubectl dies on `i/o timeout` — that is **not** a broken tailnet, it is the wrong socket. Don't switch profiles or re-login (one profile is active at a time; switching drops the personal nodes).

  ```bash
  TSW="--socket=/var/run/tailscale-work/tailscaled.sock"   # world-readable, no sudo
  /opt/homebrew/opt/tailscale/bin/tailscale $TSW status | grep operator
  # kubectl goes through the work daemon's own HTTP proxy — no tun2proxy, no sudo:
  ...
  kubectl --context trusk-staging-ts -n staging get deploy
  ```

  The daemon also exposes SOCKS5 on `1055`. `lsof -iTCP:1056` shows nothing without sudo (root-owned) — probe with `curl -x`, not lsof. Transparent routing (no proxy vars) is `tun2proxy-work`, which sets routes + `/etc/hosts`; when it is dead, work IPs resolve but fall through to the *personal* utun and time out. Revive with `sudo launchctl kickstart -k system/org.nixos.tun2proxy-work`, or just use the proxy above.
- **Staging without Tailscale** — `proxy-staging` (same IAP mechanics as `proxy-prod`), ctx `gke_trusk-staging-3rpyod_europe-west1_trusk-staging-gke`. Works with no tailnet at all. Binds **8889**, prod binds **8888** — the two can run side by side, and each alias exports the proxy vars for its own port.
- **Production** — no Tailscale; run `proxy-prod` once/session (IAP tunnel + HTTP proxy on `localhost:8888`; it exports the four proxy vars itself via `proxy-up`) and use ctx `gke_trusk-production-kkypwi_europe-west1_trusk-production-gke`. Socket `/tmp/trusk-production-gke-bastion.socket` = readiness signal (direct GKE ctx times out on TLS — private control plane).

> ⚠️ The two tunnels used to share `8888`; the loser of the race bound nothing while ssh still exited 0 and left its socket, so `localhost:8888` silently reached the **other** cluster. Fixed 2026-09-16 (distinct ports + `ExitOnForwardFailure=yes`). Still assert the cluster, fail-closed, before any write: [`notes/staging-access-and-writes.md`](/Users/nsimon/MyDocuments/TRUSK/notes/staging-access-and-writes.md).
...
- **Auth: use ADC, never ask for `gcloud auth login`.** The *user* credential (`gcloud auth print-access-token`) routinely needs interactive reauth and dies non-interactively (`Reauthentication failed. cannot prompt during non-interactive execution`) — that is NOT a blocker and never a reason to hand the user a login command. `gcloud auth application-default login` is set up and long-lived, and gcloud honours it when passed explicitly. Export it once per Bash call (mint it OUTSIDE the proxy, then set the proxy vars) and both `proxy-prod`/`get-credentials` and kubectl work:
  ...
  ```bash
  export CLOUDSDK_AUTH_ACCESS_TOKEN=$(unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY; gcloud auth application-default print-access-token)
  export http_proxy=localhost:8888 https_proxy=localhost:8888 HTTP_PROXY=localhost:8888 HTTPS_PROXY=localhost:8888
  kubectl --context gke_trusk-production-kkypwi_europe-west1_trusk-production-gke get ns
  ```

  All four casings on purpose — the lowercase pair alone sends kubectl to the Aperture shim (bullet below). Env doesn't persist across Bash calls → re-export in each. Token expires ~1h. Verified 2026-07-30 end-to-end (get-credentials → IAP tunnel → kubectl → `exec … node pg`) with the user cred fully expired.
- **gcloud token refresh fails under proxy** (`print credential failed … proxy URL malformed`, or kubectl `connection refused` to 10.0.0.2): mint any token DIRECT (`unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY` in a subshell, as above), then set the proxy vars for kubectl.
- **The lowercase `https_proxy` alone does NOT reach the prod control plane.** The shell already exports `HTTPS_PROXY=http://127.0.0.1:18888` (the Aperture shim). Go — so kubectl — reads `HTTPS_PROXY` **before** `https_proxy`, so exporting only the lowercase pair leaves kubectl pointed at the shim, which does not route to `10.0.0.2` → `net/http: TLS handshake timeout`, on every retry. The trap is that the tunnel looks perfectly healthy: `curl -x http://localhost:8888 https://10.0.0.2/version -k` returns the real `/version` payload, and `curl -x http://localhost:8888 https://www.googleapis.com/generate_204` returns 204. Symptom to recognise: curl through `-x` works, kubectl times out. Fix = export **all four** casings (`http_proxy https_proxy HTTP_PROXY HTTPS_PROXY`), and unset all four when minting the ADC token. Verified 2026-08-25.
- **proxy-prod socket goes stale**: the socket file lingers but the tunnel dies (`connection refused`). Test with a real `kubectl get ns`, not socket existence; re-run `zsh -ic 'proxy-prod'` if refused. **The converse also happens — the socket can vanish *with* the tunnel** (2026-09-15: socket absent, `curl -x` returning `000`, ADC token still valid). So socket-present proves nothing *and* socket-absent proves nothing either; only an active probe settles it. Corollary for a long session: the tunnel died three times in ten minutes after holding three hours, and a relaunch backgrounded with `&` inside a foreground Bash call **does not survive that call** — use `Bash(run_in_background:true)`. Conversely, `proxy-prod` exiting 0 with `bind [127.0.0.1]:8888: Address already in use` + `ControlSocket … already exists` means a **working** tunnel is already up — that is not the failure, don't go killing ssh processes. Probe the proxy itself first: `curl -s -o /dev/null -w '%{http_code}\n' -x http://localhost:8888 https://www.googleapis.com/generate_204` → 204 means the tunnel is fine and the problem is elsewhere (usually the `HTTPS_PROXY` casing above).
