...
Start it through a tracked background terminal process. Keep Jupyter authentication
enabled; discover the server and its token locally without printing credentials
into chat or committing them. Verify readiness using server discovery before
creating a session. Never expose an unauthenticated notebook server.
...
session via the Jupyter REST API. Resolve `JUPYTER_TOKEN` privately from the
local server discovery output before this authenticated request:
```
