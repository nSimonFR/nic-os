...
let
  ...
  prepConfigScript = pkgs.writeShellScript "claude-rc-prep-config" ''
    ...
    # Also rewrite _comment_env: the source copy says ANTHROPIC_BASE_URL
    # "deliberately does NOT live here", which is true of ~/.claude but the exact
    # opposite of this generated file — a reader landing in ~/.claude-rc would be
    # told the entry it is looking at cannot exist.
    jq '.env.ANTHROPIC_BASE_URL = "https://api.anthropic.com"
        | ._comment_env = "GENERATED by claude-remote-control.nix — edit home/dotfiles/claude-settings.json instead. ANTHROPIC_BASE_URL IS set here, unlike in ~/.claude: claude-code >= 2.1.x hard-refuses any endpoint but api.anthropic.com, and a settings-file env entry is the only thing that outranks the Aperture gate URL the wrapper defaults to. That is what keeps Remote Control working for the bridge and the sessions it spawns."' \
