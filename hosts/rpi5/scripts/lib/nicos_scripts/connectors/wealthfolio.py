...
# Sure account name -> the Wealthfolio account it is folded into, and the cost
# basis for positions Sure cannot price. BOTH ARE PERSONAL DATA, so both come
# from the agenix env at runtime and neither is written here: this repo is
# public, and a wallet identifier published beside an exact holding is
# resolvable to the full address on-chain.
#
# WEALTHFOLIO_WALLET_GROUPS  {"<sure account>": "<wealthfolio account>", ...}
# WEALTHFOLIO_COST_BASIS     {"<account>|<symbol>": <total in the position's
#                             quote currency>, ...}
#
# Both default to empty, which is the safe shape: no grouping (every Sure
# account maps to itself) and no override (Sure's own basis, or none).
WALLET_GROUPS = {}
COST_BASIS_OVERRIDES = {}


def load_personal_config(env=None):
    """Populate the grouping and basis tables from the environment."""
    global WALLET_GROUPS, COST_BASIS_OVERRIDES
    WALLET_GROUPS = json.loads(env_str("WEALTHFOLIO_WALLET_GROUPS", "{}", env) or "{}")
    COST_BASIS_OVERRIDES = {
        tuple(k.split("|", 1)): float(v)
        for k, v in json.loads(
            env_str("WEALTHFOLIO_COST_BASIS", "{}", env) or "{}").items()
    }
...
def group_accounts(accounts):
    ...
    seen: a grouped wallet can hold most of its accounts in one currency and a
    staking account in another, so picking arbitrarily would flip the whole
    wallet's denomination depending on sort order. The positions carry their own currency regardless — this is
...
def merge_positions(rows):
    ...
    Necessary, not defensive: a wallet can hold a coin at its own address AND
    the same asset via a staking account — two Sure accounts, one holding. Left
...
def sure_alternatives(cfg, run):
    ...
    number with no history, so the app cannot say the property is up an amountk
    or that an amountk of the loan is repaid — which is most of why they are
